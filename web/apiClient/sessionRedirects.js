/**
 * API 클라이언트(@/apiClient/client.js)가 감지하는 세션 이벤트를 실제 라우팅으로 연결한다.
 *
 ...
 * useNavigate 를 쓸 수 없다. web/app/providers.tsx 가 Next.js 의 useRouter 를 감싸
 * 만든 어댑터 객체(router.navigate/router.state)를 사용해 클라이언트 라우팅과
 * 세션 스토어를 유지한 채 이동한다(window.location 은 전체 새로고침을 일으켜
 * 스토어를 날린다).
 *
 * import 를 상대 경로로 쓴다: client.js 는 자체 import 가 없어 어느 테스트
 * 러너에서도 그대로 로드된다.
 */
...
/**
 * 세션 이벤트 리스너를 라우터에 등록한다. web/app/providers.tsx 가 컴포넌트의
 * useRef 로 가드한 useEffect 안에서 한 번만 호출하므로 React StrictMode 의
 * 이중 effect 와 무관하다. 리스너 setter 는 단순 대입이라 재등록도 안전하지만,
 * 이미 목적지에 있는 경우에는 중복 이동을 하지 않도록 경로를 먼저 확인한다.
 *
 * @param {object} params
 * @param {{ navigate: Function, state: object }} params.router providers.tsx 가 만든 라우터 어댑터
