...
9. 정답 없음 → "정답은 필수입니다."
10. 이미지 열이 비어 있지 않음 → "이미지는 엑셀로 등록할 수 없습니다. 이미지 열은 비워 두고, 문제 개별 등록/수정 화면에서 이미지를 첨부하세요."
11. 보기/정답 유형별 검증(아래)
