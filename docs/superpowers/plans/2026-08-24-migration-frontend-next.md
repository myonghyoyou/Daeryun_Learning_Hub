...
- 테스트는 **한 러너(vitest)** 로 모은다. Task 2 완료 시점 실측 기대치는 **985개**(web 714 + 이관 271 — 브리프의 254 는 사전 집계 오차, Task 2 리뷰에서 정정됨)였고, 그 뒤 Task 5 가 죽어 있던 sessionRedirects 테스트 5개를 되살리고 Task 7 이 신규 3개를 더해 P2 종료 시점 **993개**, 통합 리뷰 수정으로 P2 마감 시점 **994개**다. **숫자를 고정값으로 대조하지 말고 매 태스크 검증 시 직전 값보다 줄지 않았는지(무회귀)로 판단한다.**
  ~~P3·P4 가 화면마다 테스트를 추가하므로 이 값은 계속 늘어난다~~ — **틀린 예상이었다**(최종
  리뷰에서 발견, 2026-08-26). 실제로는 P3·P4 6개 태스크(Task 8~13) 전부 순수 이동
  작업(라우트 파일은 화면 컴포넌트를 렌더하는 얇은 껍데기, `sessionChecked` 같은 손으로
  짠 로직도 이미 Task 10 자체 테스트로 커버됨)이라 994 에서 변하지 않았고, 이는 회귀가
  아니라 각 태스크 리뷰에서 확인된 정상 상태다. Task 16 이 993→995 로 늘린 것은 G11
  단위 테스트 신규 작성이었고, 최종 리뷰 수정(I-1 대응)이 995→999 로 늘렸다
  (`web/apiClient/routerAdapter.test.js` 신규 4개, 커밋 `28c6fb6`).
...
| 구간 | 태스크 | 내용 | 상태 |
|---|--:|---|---|
| **P1 기반** | 3 | 스타일·순수 모듈·공통 부품 이동. 화면은 아직 없다 | ☑ 2026-08-25 |
| **P2 가드·인증** | 4 | 가드 4개 → 레이아웃, 화면 3개, F3 수정, not-found | ☑ 2026-08-25 |
| **P3 풀이** | 3 | `/solve/**` 7화면 | ☑ 2026-08-25 |
| **P4 관리자** | 3 | `/admin/**` 9화면 | ☑ 2026-08-26 |
| **P5 마감** | 3 | X 절 검증 · `frontend/`·`backend/` 제거 · 검증 문서 | ☑ 2026-08-26 |
