...
프론트엔드 문제 풀이 화면은 `docs/superpowers/specs/2026-07-29-blue-bento-design-system.md`를 단일 기준으로 사용한다. 대상 화면은 디자인 시스템 8.2 학습 홈, 8.3 문제 목록, 8.4 문제 풀이, 8.5 풀이 이력과 9.2 모바일 규칙이다.

- `/solve`는 Blue Bento Learning 학습 홈 구조를 사용하고, `/solve/problems`와 `/solve/history`는 학습 Shell 안에서 이동한다. `/solve/:id`는 문제 번호·진행률·유형·본문·제출 영역의 위계를 유지한다.
- PC는 콘텐츠 폭과 문제 풀이 집중도를 우선하고, 모바일은 1열 카드·하단 고정 제출 영역·터치 가능한 선택지 크기를 사용한다. 모바일 관리자 메뉴는 노출하지 않는다.
