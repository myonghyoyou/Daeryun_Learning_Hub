...
- Produces: `formatAccuracyRate(rate)`, `isReviewNeeded(item)`, `REVIEW_MIN_ATTEMPTS`, `REVIEW_ACCURACY_THRESHOLD`, `listProblemStats(params)`, `getProblemStatDetail(id)`, `getDashboardSummary(departmentId)`. Task 6·7이 사용한다.
