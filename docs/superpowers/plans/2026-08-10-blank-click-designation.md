...
- 현재 기준선: **백엔드 210 / 프론트엔드 189 전부 통과, 프로덕션 빌드 성공.** (구현 완료 후 프론트엔드 216 — 아래 완료 기준 참조)
...
- [x] 지정 모드에서 어절을 클릭하면 `{{bN}}` 마커 + 정답이 함께 생긴다 (이중 입력 제거)
- [x] 조사·쉼표가 자동 분리되고, 화살표로 ±1글자 보정된다
- [x] 빈칸 해제 시 정답 텍스트가 본문으로 되돌아온다
- [x] 쓰기 모드에서 본문을 고쳐도 지정 모드가 다시 파싱해 어긋나지 않는다
- [x] 수정 화면에서 기존 `{{b1}}`이 빈칸 칩으로 보인다 (추가 코드 없이) — 단 `{{bN}}` 키에 한함(아래 B3)
- [x] 백엔드 무변경 — 저장 payload가 기존과 동일 (`git diff --name-only e0b6f15..HEAD -- backend/` 비어 있음)
- [x] 프론트엔드 189 → **216** 전부 통과, 빌드 성공
- [x] 브라우저 콘솔에 React 훅 오류 없음 — B2 수정 후 재검증에서 오류 0건

> 위 "206"은 계획 작성 시점의 추정치였다. Task 3 리뷰에서 방어 분기 회귀 테스트 10건이 추가되어 실제는 **216**이다(189 + Task 1 6 + Task 2 5 + Task 3 6 + Task 3 수정 라운드 10).

### 브라우저 검증에서 결함 3건 → 수정 완료

Task 5를 실제로 수행해 결함 3건을 찾았고 Task 6에서 수정했다(전문: `docs/qa/2026-08-11-blank-designation-browser-verification.md`).

| | 결함 | 수정 |
|---|---|---|
| **B1** Critical | 같은 키의 마커가 두 번 있으면 `indexOf`가 첫 출현만 잡아, 아무 빈칸도 선언하지 않는 `{{b1}}` 고아 마커가 **검증을 통과해 저장**됐다(학습자에게 마커 날것 노출) | `releaseBlank`이 그 키의 **모든 출현**을 되돌린다. `adjustBlankBoundary`는 출현이 2개 이상이면 no-op(정답은 하나인데 각 마커 뒤 글자가 달라 성립하지 않음) |
| **B2** Critical | 같은 상황에서 `blank-${key}` React key 중복 — 콘솔에 "Encountered two children with the same key" 실제 출력 | blank 세그먼트에 `start`/`end` 오프셋을 부여하고 key를 `blank-${start}`로 |
| **B3** Important | `MARKER`가 `{{bN}}`만 인식하는데 앱 자신의 placeholder와 기존 DB 데이터는 `{{blank_1}}` 형식이라, 마커가 어절로 렌더되고 클릭 시 편집본이 깨졌다 | `MARKER`를 `/\{\{([A-Za-z0-9_-]+)\}\}/g`로 넓혀 키 형식에 종속되지 않게 |

**셋 다 216개 단위 테스트·프로덕션 빌드·태스크별 코드 리뷰·브랜치 통합 리뷰를 전부 통과한 채 남아 있었다.** 이 계획이 §Global Constraints에 "화면 배선은 브라우저 검증이 유일한 안전망"이라고 적어 둔 것이 그대로 입증됐다.

재검증도 같은 브라우저에서 수행했다 — 고아 마커 없음, 콘솔 오류 0건, `{{blank_N}}` 문항이 칩으로 정상 렌더. 프론트엔드 **221 통과**, 빌드 성공.

**남은 제약:** `validateBlanks`는 여전히 "본문의 모든 마커가 선언돼 있는가"를 검사하지 않는다. 지정 모드가 고아를 만드는 경로는 닫혔지만 본문 직접 편집·빈칸 행 삭제로는 여전히 만들 수 있다(이 브랜치 이전부터의 동작). 클라이언트만 고칠지 서버까지 고칠지 판단이 필요해 범위에서 제외했다.

### 후속 조치: QA 자격증명 문서 drift

`docs/qa/2026-08-04-plan1-2-qa-checklist.md`와 `docs/qa/2026-08-07-plan3-result.md:133`이 기록한 `admin` / `QaAdmin1234!`가 더 이상 맞지 않는다(계정은 ACTIVE·잠금 없음이므로 다른 환경에서 교체된 것으로 보이며, 새 값은 어느 문서에도 없다). 같은 체크리스트가 적어 둔 `admin2`·`deptadmin`·`emp002`·`legacy01` 네 계정은 **DB에 아예 존재하지 않는다** — 실재하는 계정은 `admin`·`dev_admin`·`sales_admin`·`emp001` 4개뿐이다.

이번 검증은 `docs/qa/2026-08-07-plan3-result.md:134`의 `dev_admin` / `QaPlan3!2026`으로 수행했다(유효 확인).
