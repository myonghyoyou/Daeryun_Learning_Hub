...
- 총 **87행** (E 7 · S 11 · P 12 · Q 14 · G 15 · T 14 · H 8 · U 6)
...
| T11 | **`blankAnswers` 에 `null` 원소가 섞인 경우 (승인된 이탈 ㉳)** | 포트: **400 / 1000 / `제출한 빈칸 개수가 올바르지 않습니다.`** — `null` 은 정의된 키가 아니므로 G10 에 걸린다 | Java 는 `submitted.stream().map(BlankAnswerInput::getBlankKey)` 에서 **NPE** → catch-all → **200 / -1**. 포트가 `-1` 을 없앤 것이고 `problemRequestBody.ts:127` 의 선례를 따른다. **파리티 위반으로 보고하지 마라** |
...
> 저장된 `image_url` 이 이미 `/api/problem-images/<uuid>.<ext>` 이고 미들웨어가 `/api/:path*` 를
> 세션 게이트하므로, 저장값을 그대로 쓰면서 게이트가 공짜다. 서명 URL 은 만료돼 저장할 수 없어
> 이미지 URL 을 싣는 **모든 응답을 변환**해야 한다. 근거 전문은 계획서 Task 6.
> **아래 서술은 결정 당시의 검토 기록으로 남긴다 — 열린 질문이 아니다.**
...
| ㉱ | 비공개 버킷 이미지 조회 | 로컬 디스크를 정적 리소스로 서빙 | **프록시 라우트** — 위 결정 참고 | **승인됨**(계획서 Task 6) |
| ㉳ | `blankAnswers` 의 `null` 원소(T11) | NPE → 200/-1 | 400/1000/`제출한 빈칸 개수가 올바르지 않습니다.` | **승인됨** — `-1` 제거는 이 이관의 목적이다 |
