...
| F7 | 파일 크기 상한(승인된 이탈③) | 파일 크기 초과 | Spring 20MB(멀티파트) → 4MB로 하향, resultCode 1015. **M7 실측(2026-08-21):** 4.2MB·5MB·8MB·9MB·10,481,664 B 는 전부 400/1015 "파일 크기가 허용 범위를 초과했습니다."로 맞는다. 그러나 요청 본문이 **10 MiB(10,485,760 B)** 이상이면 라우트가 `MAX_FILE_BYTES` 를 재기 **전에** `await request.formData()` 가 실패해 이탈 ⑥ 경로로 빠지고 200/1009 "파일을 업로드할 수 없습니다."가 나간다(10,485,759 B 부터 재현). 거절된다는 결과는 같지만 문구·상태·코드가 다르다 — **컷오버 이월**(배포 플랫폼의 실제 상한은 이보다 훨씬 낮을 수 있어 그때 다시 재야 한다). 순서를 뒤집어 고치지 않는다: "파싱 실패 → 1009"는 형제 라우트 셋이 공유하는 스탠딩 룰링이다 | `application.yml:18-19`; 포트: `web/app/api/admin/problems/excel-upload/route.ts`(`request.formData()` → `MAX_FILE_BYTES` 순서) |
