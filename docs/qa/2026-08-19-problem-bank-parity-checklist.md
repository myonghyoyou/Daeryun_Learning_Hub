...
### ⑧ 공백 판정이 JS 기준이다 — Java 는 U+0020 이하만 깎는다 (2026-08-24 추가, 실측)

`problemValidation.ts` 의 `isBlank`·`trimToNull`·`normalizeTags` 가 JS `.trim()` 을 쓴다.
Java 원본은 `String::trim`(`ProblemServiceImpl.java:264`)과 그 위에 얹힌 `trimToNull` 이며,
**코드포인트 U+0020 이하만** 깎는다. JS `trim()` 은 U+00A0(비분할 공백)·U+3000(전각 공백)까지 깎는다.

**Spring 실측 (문제 생성 후 저장된 코드포인트를 DB 에서 직접 읽음):**

| 입력한 본문/태그 | Java 가 저장한 값 |
|---|---|
| `U+00A0` + `가나다` + `U+00A0` | `00a0 ac00 b098 b2e4 00a0` — **NBSP 가 양쪽에 그대로 남는다** |
| `U+00A0` **하나뿐** | `00a0` — **저장된다.** Java 는 이걸 blank 로 보지 않는다 |
| `U+3000` + `전각본문` | `3000 …` — 전각 공백이 남는다 |
| 태그 `U+00A0` + `공백태그` + `U+00A0` | `00a0 공백태그 00a0` — **태그도 그대로** |
| `U+0020` 하나뿐 (대조군) | 거부 — `문제 내용을 입력하세요.` |

**포트의 동작:** 위 넷이 전부 다르다. 앞뒤 NBSP 는 제거되고, NBSP 하나뿐인 본문은
`문제 내용을 입력하세요.` 로 거부되며, 태그는 `공백태그` 로 정규화된다.

**코드를 바꾸지 않는다(사용자 승인, 2026-08-24).**
- 사용자에게 해로운 방향이 아니다 — 붙여넣기로 섞인 보이지 않는 공백이 **정리되어** 저장된다.
- 서브플랜 4는 정답지 138행 + E2E 로 검증이 끝난 상태다. 이 차이를 맞추려고 검증된
  검증 로직을 흔드는 비용이 얻는 것보다 크다.
- **다만 태그는 성질이 다르다.** 태그는 이름으로 중복 제거되므로, 같은 사용자 입력이
  Java 와 포트에서 **서로 다른 `tags` 행**을 만든다. 운영 데이터를 Spring 에서 이관하는 일이
  생기면 이 차이가 태그 중복으로 나타난다 — **컷오버 목록에 올린다.**

**같은 부류를 이미 고친 곳:** 서브플랜 5의 채점 경로(`lib/solve/grading.ts` 의 `javaTrim`).
거기는 **채점 결과가 갈리므로** 맞췄다. 판단 기준은 "차이가 사용자에게 보이는가"다.
...
| I4 | 크기 상한 | 파일 크기 > 5MB | "이미지 크기는 5MB를 초과할 수 없습니다." — **주의(값은 바꾸지 않는다, Java 와의 일치가 우선):** 이 애플리케이션 검사(5MB)는 플랫폼 요청 본문 상한 위에 얹혀 있을 뿐, 그 상한을 대신하지 않는다. 엑셀 두 라우트는 같은 서버리스 본문 상한을 이유로 Spring 20MB → 4MB 로 낮추는 승인된 이탈을 기록했다(이 문서 F7/이탈③, `docs/qa/2026-08-16-dept-users-parity-checklist.md` 이탈④) — M6 는 5MB 를 그대로 유지해 그 이탈 이유를 적용하지도, 명시적으로 무효화하지도 않았다(추론의 불일치). 결과적으로 대략 4~5MB 구간의 파일은 이 애플리케이션 검사(87행)가 실행되기도 전에 플랫폼이 일반 413 으로 거절하고, 사용자는 위 "이미지 크기는 5MB를 초과할 수 없습니다." 문구를 절대 보지 못한다. localhost 에는 이런 요청 본문 상한이 없으므로 이 구간은 **로컬에서 검증 불가능**하다 — M7 은 이 구간을 초록으로 인증하지 말고 컷오버 항목으로 기록해야 한다. 정확한 플랫폼 상한값은 이 저장소 어디에도 문서화되어 있지 않으며 컷오버 시점에 실측해야 한다. **M7 실측(2026-08-21):** 애플리케이션 검사 자체는 맞다 — 5,242,881 B → 400/1015 "이미지 크기는 5MB를 초과할 수 없습니다.", 정확히 5,242,880 B → 200. 4~5MB 구간은 지시대로 **초록으로 인증하지 않고 컷오버로 이월**한다. **정정(같은 웨이브):** "localhost 에는 요청 본문 상한이 없다"는 이 행의 원래 전제는 부정확했다 — 로컬에도 상한이 있다. `web/middleware.ts` 의 matcher(`/api/:path*`)가 잡는 모든 API 요청은 `next-server.js:1320-1321` 에서 `experimental.middlewareClientMaxBodySize`(기본값 `config-shared.js:219` 의 10,485,760 B = 10 MiB)로 감싸이고, 이를 넘으면 `body-streams.js:85-97` 이 예외 없이 본문을 **자른다**(런타임이 던지는 파싱 상한이 아니라, `next.config.mjs` 로 조정 가능한 앱 설정이 기본값으로 적용된 것 — 앱과 함께 배포 플랫폼까지 이동한다). 이 상한은 4~5MB 대역보다 훨씬 위(10 MiB)에 있으므로 **이 행이 가리키는 4~5MB 대역 자체에는 영향을 주지 않는다** — 4~5MB 대역이 로컬에서 검증 불가능하다는 결론은 그대로 유효하다(플랫폼의 더 낮은 상한을 로컬이 재현하지 못하는 것이 이유다). 10 MiB 잘림 자체는 별도의 컷오버 항목이다(F7 참고, 검증 문서 "실측으로 새로 드러난 것" 1·2와 "컷오버 핸드오프") | `ProblemImageServiceImpl.java:45,63-65` |
...
| I11 | 파일 필드 부재/빈 파일 | `file`이 없거나 빈 파일 | resultCode 1009(`FILE_REQUIRED`) — 단, **세 갈래가 서로 다른 Java 경로를 탄다**(파트가 "없는" 것과 "0바이트로 있는" 것을 같은 취급으로 뭉개면 안 된다). ⓐ 멀티파트 자체를 파싱할 수 없음(Spring `MultipartException`, 인터셉터·컨트롤러 진입 전): 고정 문구 "파일을 업로드할 수 없습니다."(200). **이 ⓐ 는 "멀티파트가 깨져 있음"과 "바디가 애초에 멀티파트가 아님"이라는 서로 다른 두 하위 경우를 하나로 뭉뚱그린다 — Java 에서는 이 둘이 다른 결과를 낸다.** ⓐ-1(멀티파트지만 깨져 있음, 예: `Content-Type: multipart/form-data`인데 바디가 손상): `DispatcherServlet.checkMultipart` 가 `multipartResolver.isMultipart(request)`(`true`) 뒤에 `resolveMultipart` 를 호출하다 `MultipartException` 을 던지고, 인터셉터·컨트롤러 진입 전이라 역할 검사(R1)도 타지 않는다 — 위 문구·200 그대로. ⓐ-2(바디가 애초에 멀티파트가 아님, 예: `Content-Type` 자체가 multipart 가 아님): `isMultipart(request)` 가 `false` 라 `checkMultipart` 는 예외를 던지지 않고 요청을 그대로 통과시킨다(바이트코드로 확인: `isMultipart` 가 거짓이면 `resolveMultipart` 호출 없이 곧장 리턴). 즉 이 경우는 정상적으로 `RoleCheckInterceptor.preHandle`(I13)까지 흘러가 role 이 맞지 않으면 **403/990**이, role 이 맞으면 그 뒤 `@RequestParam("file") MultipartFile` 바인딩 실패 경로(ⓑ와 유사하되 별도로 실측 필요)가 이어진다 — 어느 쪽이든 ⓐ-1과 결과가 다르다. **포트는 이 구분을 두지 않는다**: `request.formData()` 를 먼저 호출해 실패하면(멀티파트 여부와 무관하게) ⓐ-1·ⓐ-2 를 하나의 `fileMissing()`(200/1009/"파일을 업로드할 수 없습니다.")으로 통합한다. 코드는 그대로 둔다 — 형제 라우트 셋(계정/문제 엑셀 업로드, 이미지 업로드)이 전부 이 취급으로 일치하고, 하나만 고치면 그 일관성이 깨진다(스탠딩 룰링, 이 이슈 자체를 정정하지 않는다). ⓐ-2(비-멀티파트 바디)는 role 이 맞는 요청에서 어떤 문구가 나가는지까지는 이 항목이 실측하지 않았으므로 **컷오버 항목으로 이월**한다 — 이 checklist 항목은 ⓐ-1(깨진 멀티파트)만 검증했다는 뜻이고, M7 은 ⓐ 를 초록으로 인증할 때 이 구분을 근거로 남겨야 한다. ⓑ 파싱은 됐지만 `file` **파트가 아예 없음**: `@RequestParam("file") MultipartFile` 바인딩 자체가 `MissingServletRequestPartException`으로 실패하고, `GlobalExceptionHandler`엔 이 예외 전용 핸들러가 없어 **catch-all로 떨어져 200/`-1`/"처리 중 오류가 발생하였습니다."**가 나간다 — 서비스의 `file == null` 가드(`ProblemImageServiceImpl.java:60-62`)는 컨트롤러 바인딩 실패 뒤라 **도달 불가능**하다. **포트는 이 `-1`을 그대로 재현하지 않는다** — 형제 라우트(계정/문제 엑셀 업로드)와 동일하게 200/1009/"파일을 업로드할 수 없습니다."로 안내하는 **승인된 이탈 ⑥**을 적용한다(ⓐ와 문구가 같아진다). **M7 실측(2026-08-21):** 포트 쪽 네 갈래를 전부 쟀다 — ⓐ-1(깨진 멀티파트) 200/1009, ⓐ-2(비-멀티파트 바디) 200/1009, ⓑ(파트 부재) 200/1009, ⓒ(0바이트) 400/1009 "필수 파일이 누락되었습니다.". ⓐ-2 는 **역할이 틀린 요청(EMPLOYEE)에서도 200/1009** 였다 — Java 라면 `RoleCheckInterceptor` 까지 흘러가 403/990 이 났을 자리다. 이 차이 자체는 위 스탠딩 룰링대로 정정하지 않는다. 이월로 남는 것은 "role 이 맞는 ⓐ-2 요청에서 **Java 가** 실제로 어떤 문구를 내는가" 하나뿐이고, 그건 Spring 인스턴스가 있어야 재므로 컷오버 항목이다. ⓒ `file` **파트는 있지만 0바이트**: 이 경우에만 실제로 `MultipartFile.isEmpty()` → `BizException(ErrorCode.FILE_REQUIRED)`에 도달한다 — 커스텀 메시지가 없으므로 `ErrorCode` 기본 문구 "필수 파일이 누락되었습니다."가 나가고, `EMPTY_SESSION`·`ACCESS_AUTH_DENIED`가 아니므로 **HTTP 400**이다(이건 그대로 이식, 이탈 아님) | ⓐ `GlobalExceptionHandler.java:74-77`(`MultipartException`); ⓐ-1·ⓐ-2 구분: `DispatcherServlet.checkMultipart` — `isMultipart(request)` 가 `false` 면 `resolveMultipart` 를 아예 호출하지 않고 리턴(spring-webmvc 5.3.22 바이트코드로 확인, 2026-08-19), 그러면 `RoleCheckInterceptor.java:22-46`(`preHandle`, role 불일치 시 `ACCESS_AUTH_DENIED`→403/990)까지 정상 진입; ⓑ `GlobalExceptionHandler.java:85-89`(catch-all), `ProblemImageServiceImpl.java:60-62`(도달 불가능한 가드); 이탈 ⑥ 근거: `docs/qa/2026-08-16-dept-users-parity-checklist.md:19`, `docs/superpowers/plans/2026-08-16-migration-dept-users.md:23`; ⓒ `ProblemImageServiceImpl.java:60-62`, `exception/BizException.java:11-13`(인자 없는 생성자는 `errorCode.getMessage()`), `GlobalExceptionHandler.java:31-38`(400); 포트: `web/app/api/admin/problems/images/route.ts` `fileMissing()`(ⓐ·ⓑ 통합)과 `emptyFile()`(ⓒ, `bizStatus()`로 400 산출) |
| I12 | 기존 데이터 영향 | 이관 시점 `problems.image_url` 현황 | NULL이 아닌 행 0건(26건 중 0) — 접두어 변경(승인된 이탈②)이 기존 데이터를 깨지 않는 근거. **이 값은 2026-08-19 시점의 역사적 실측치이므로 M7 의 재측정 대상이 아니다.** M7 검증은 개발 DB 에 NOT NULL 행 2건을 새로 만들었다(`/api/problem-images/7d512a3e-….png` = Step 3 에서 지운 오브젝트를 가리키는 dangling 참조, `/api/problem-images/a.png` = X11 "이미 유효한 경로는 그대로 저장된다" 확인용 값) — 서브플랜 5 가 조회 경로를 만들 때 이 둘이 404 를 내는 것은 정상이다 | Spring 코드 출처 아님 — DB 실측(2026-08-19), plan Global Constraints 원문에 기록; M7 실측 `docs/qa/2026-08-19-problem-bank-e2e-verification.md` |
...
| I14 | 버킷·환경변수(M6) | Supabase Storage 설정 | 비공개 버킷 `problem-images`(5MB 상한, `png/jpeg/gif/webp`만, svg 제외 — 버킷 자체 정책과 애플리케이션 검증(I1·I4)이 이중으로 막는다). 서버 전용 환경변수 `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` — 서비스 롤 키는 `NEXT_PUBLIC_` 접두어를 절대 붙이지 않는다(붙이면 브라우저 번들에 들어가 누구나 스토리지 전체를 조작할 수 있다). `web/.env.example` 이 삭제되어 있어 이 표가 새 환경변수의 기록처다 — `TEST_DATABASE_URL`(테스트 전용 DB 접속 문자열; `web/test/db.ts`가 이름이 `_test`로 끝나는지 강제하고, 아니면 시끄럽게 실패한다)도 여기 함께 기록한다. **테스트는 `.env`를 읽지 않는다** — 절대로 테스트 엔트리포인트에 `import "dotenv/config"`를 쓰지 말 것: 그러면 앱의 `DATABASE_URL`(개발 DB)까지 `process.env`에 실리고, vitest는 `fileParallelism:false`라 전 테스트 파일이 프로세스 하나를 공유하므로 그런 파일이 하나만 있어도 스위트 전체의 `testDb()`가 개발 DB를 향하며 `truncateAll`이 13개 테이블을 그 DB에서 CASCADE 로 비운다 — 실제로 그렇게 됐었다(`36944f2`). `SUPABASE_URL`·`SUPABASE_SERVICE_ROLE_KEY`는 값이 있기만 하면 되므로(스토리지 클라이언트는 목이다) `web/test/env.ts`가 테스트 전용 가짜 값을 직접 세팅한다 — 자세한 내용과 근거는 `web/test/env.ts`·`web/test/db.ts` 상단 주석을 볼 것 | Spring 코드 출처 아님(승인된 이탈①의 플랫폼 대체) — 이관 스펙 Q8; 포트: `web/lib/problem/problemImage.ts`(`getStorageClient`), `web/test/env.ts`(테스트 전용 값 세팅), `web/test/db.ts`(`TEST_DATABASE_URL` 가드) |
...
| F7 | 파일 크기 상한(승인된 이탈③) | 파일 크기 초과 | Spring 20MB(멀티파트) → 4MB로 하향, resultCode 1015. **M7 실측(2026-08-21):** 4.2MB·5MB·8MB·9MB 파일(요청 본문은 각각 10 MiB 보다 작다)은 전부 400/1015 "파일 크기가 허용 범위를 초과했습니다."로 맞는다. 그러나 미들웨어(matcher `/api/:path*`)가 감싸는 본문 클론 상한 — 기본값 정확히 10 MiB(`config-shared.js:219`) — 을 요청 **본문 전체**가 넘으면, 라우트가 `MAX_FILE_BYTES` 를 재기 **전에** Next 가 본문을 자르고(`body-streams.js:85-97`, 예외 없음) 멀티파트 경계가 깨져 `await request.formData()` 파싱이 실패한다 — 이탈 ⑥ 경로로 빠져 200/1009 "파일을 업로드할 수 없습니다."가 나간다. **정정(M7):** 경계는 본문 바이트 기준 `> 10,485,760 B` 다 — 본문이 정확히 10,485,760 B(=10 MiB)면 400/1015(앱 검사에 정상 도달), 10,485,761 B 부터 200/1009 로 넘어간다(이전 판이 적었던 `10,485,759 B`는 이 경계에서 재확인한 값이 아니라 파일 바이트였다). 거절된다는 결과는 **멀티파트에 한해** 같지만 문구·상태·코드가 다르고, 비-멀티파트 라우트는 거절조차 보장하지 않는다(검증 문서 "실측으로 새로 드러난 것" 2 참고) — **컷오버 이월**(배포 플랫폼의 실제 상한은 이보다 훨씬 낮을 수 있고, 그 값과 이 10 MiB 상한을 같은 세션에서 함께 재야 한다). 순서를 뒤집어 고치지 않는다: "파싱 실패 → 1009"는 형제 라우트 셋이 공유하는 스탠딩 룰링이다 | `application.yml:18-19`; 포트: `web/app/api/admin/problems/excel-upload/route.ts`(`request.formData()` → `MAX_FILE_BYTES` 순서); Next: `web/middleware.ts`(matcher), `next-server.js:1320-1321`, `config-shared.js:219`, `body-streams.js:85-97` |
...
**M7 대조 완료(2026-08-21).** 138행 전부를 실측과 맞춰 봤고 결과는 `docs/qa/2026-08-19-problem-bank-e2e-verification.md` 에 행별로 있다: **실측 확인 133행 · 미실측 5행**(N6·C8 = 레이스 재현 실패, N8 = HTTP 경로에서 도달 불가, I8 = 감사 실패를 E2E 로 유도할 수 없음, I12 = 이관 시점 실측치라 재측정 대상 아님) · **컷오버 이월 3건**(I4 4~5MB 구간, I11ⓐ-2 의 Java 쪽 응답, F7 의 ≥10 MiB 구간). 승인된 이탈 7건은 전부 유지했고 재심하지 않았다. **프로덕션 코드 결함은 나오지 않았다.**
