...
| I11 | 파일 필드 부재 | `file`이 없거나 빈 파일 | resultCode 1009(`FILE_REQUIRED`). **두 가지 문구가 같은 코드 아래 공존한다** — ⓐ 멀티파트 자체를 파싱할 수 없음(Spring `MultipartException`, 인터셉터·컨트롤러 진입 전): 고정 문구 "파일을 업로드할 수 없습니다.". ⓑ 파싱은 되었지만 `file` 파트가 없거나 빈 파일(Spring `MultipartFile.isEmpty()`): `BizException(ErrorCode.FILE_REQUIRED)`에 커스텀 메시지가 없으므로 `ErrorCode` 기본 문구 "필수 파일이 누락되었습니다."가 그대로 나간다. 두 분기를 하나의 문구로 뭉뚱그리면 ⓑ 케이스에서 문구가 어긋난다 | `ProblemImageServiceImpl.java:60-62`; `ErrorCode.java:10`; `GlobalExceptionHandler.java:74-77`(ⓐ, `MultipartException` → 고정 문구); `exception/BizException.java:11-13`(ⓑ, 인자 없는 생성자는 `errorCode.getMessage()`); 포트: `web/app/api/admin/problems/images/route.ts` `multipartUnreadable()`(ⓐ)와 `fileMissing()`(ⓑ) |
