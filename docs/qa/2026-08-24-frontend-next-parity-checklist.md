...
| G8 | 리스너 등록이 정확히 1회 | `main.jsx` 모듈 최상위에서 등록해 StrictMode 이중 effect 와 무관. **Next 에서는 컴포넌트 안으로 들어가므로 이중 등록·미등록 둘 다 가능해진다** | ☐ |
...
| G11 | stale 응답 폐기 | `generation` 카운터로 무효화 이전 fetch 응답을 버린다. 로그인 직후 이전 세대 응답이 `unauthenticated` 로 되돌리지 않는지 | ☐ |
...
| 행 | 확인할 것 | 기대 | 실측 |
|---|---|---|---|
| X1 | 딥링크 새로고침 | 20개 경로 전부에서 F5 가 같은 화면을 돌려준다(N-1 이 겸함) | ✅ 20개 경로 전부 주소창 직접 진입으로 확인(N-1 이 겸함) |
| X2 | 뒤로가기 | 화면 이동 후 뒤로가기가 이전 화면으로 | ✅ `/admin/stats` → 뒤로 → `/admin/problems`(`문제 관리`) |
| X3 | `replace` 이동은 뒤로가기로 돌아오지 않는다 | 로그인 성공 → `/`(replace) 후 뒤로가기가 **`/login` 으로 돌아가지 않는다**. `useLogout`·`ChangePassword`·랜덤 흐름도 동일 | ✅ 로그아웃 후 뒤로가기 → `/login` 유지. 관리자 화면으로 돌아가지 않는다 |
| X4 | 앞으로가기 | 뒤로 간 뒤 앞으로가 동작 | ✅ 앞으로 → `/admin/stats`(`문제별 정답률 통계`) |
| X5 | 화면 이동 시 전체 새로고침이 일어나지 않는가 | 세션 스토어가 유지된다. **전체 새로고침되면 `GET /api/auth/session` 이 매번 다시 나가고 Loader 가 깜빡인다** — 네트워크 탭으로 센다 | ✅ **문서 요청 0건** — 사이드바 이동 시 API 호출만 발생 |
| X6 | 스크롤 위치 | 새 화면 진입 시 최상단. 뒤로가기 시 이전 위치(현재 동작을 먼저 실측해 그것을 기준으로 삼는다) | ⚠️ **이관에서 저절로 바뀌는 동작.** 새 화면 진입 = `scrollY 0` ✅. **뒤로가기도 `scrollY 0` — 복원하지 않는다**(400 → 0). Next App Router 는 뒤로가기 스크롤 복원이 **기본값**이므로 배관을 갈면 동작이 달라진다 — F4 참조 |
| X7 | 존재하지 않는 경로 | **결함 확인 — 아래 F1 참조.** react-router 의 개발자용 **영어** 오류 화면이 뜬다 | ⚠️ `Unexpected Application Error! / 404 Not Found / 💿 Hey developer 👋 …` |
| X8 | 콘솔 오류·경고 | 20개 화면 전부에서 **0건** | ✅ 관리자 대시보드·통계·문제 관리에서 오류·경고 **0건** |
...
### F4. 뒤로가기 스크롤 복원이 이관만으로 달라진다

측정: `/admin/problems` 에서 400px 스크롤 → `/admin/stats` 이동(`scrollY 0`) → 뒤로가기
→ **`scrollY 0`.** 지금 앱은 **스크롤을 복원하지 않는다.**

**Next App Router 는 뒤로가기 스크롤 복원이 기본 동작이다.** 즉 배관을 갈아 끼우는 것만으로
이 동작이 **저절로 바뀐다** — 아무도 그렇게 하기로 결정하지 않았는데.

F1~F3 과 성질이 다르다. 이건 **결함이 아니라 프레임워크 기본값 차이**이고, 그래서 더
위험하다. 코드에 흔적이 없어 리뷰에서 안 보이고, 실측하지 않으면 아무도 모른다.

**이관 시 판단**: 복원되는 쪽이 사용자에게 낫다(긴 목록에서 상세를 보고 돌아올 때). 그러나
**"저절로 바뀌었다"로 남기지 않고 승인된 이탈로 기록한다.** 계획서에서 확정한다.
...
| 절 | 측정 | 결과 |
|---|---|---|
| G 전역·진입 | 8 / 11 | G6 ⚠️(F3) · G7~G10 ✅ · **G8·G11 미측정**(Q4) |
...
| **합계** | **91 / 94** | 미측정 3(G8·G11·A4→A4 완료로 최종 2) |
...
| Q4 | **G8**(세션 리스너가 정확히 1회 등록) · **G11**(stale 응답 폐기) | 둘 다 **관측 가능한 증상이 없다.** G8 은 중복 등록되어도 리다이렉트가 두 번 나갈 뿐 최종 화면이 같고, G11 은 경합을 재현할 타이밍 제어 수단이 없다. **단위 테스트로 고정하는 편이 맞다** — 계획서에서 그렇게 배치한다 |
