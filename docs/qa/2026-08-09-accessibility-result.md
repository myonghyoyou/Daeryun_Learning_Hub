...
- **커밋:** `7eb5dfa`(D5) · `899309f`(D6), 브랜치 `fix/accessibility-d5-d6`
- **방식:** TDD(RED 확인 → 최소 수정 → GREEN) + Vibescraper 실측

### D5 — 200% 확대 (§11.11) ✅ 해소

`MOBILE_BREAKPOINT` 768 → **640**. 판별 함수는 `utils/device.js` 한 곳뿐이고 소비처가 `AdminRoute`·`Landing` 둘뿐이라 호출부 변경이 없었다.

RED로 먼저 고정한 것: `classifyDevice(720)`이 `"mobile"`을 돌려주는 것을 확인한 뒤 상수를 바꿨다.

| 뷰포트 | 의미 | 결과 |
|---|---|---|
| 1440 / 960 / 720 | 100% / 150% / **200% 확대** | `/admin/problems` 유지 ✅ |
| 640 | 임계값 | 유지 ✅ |
| 639 | 임계값 바로 아래 | `/solve` 차단 ✅ |
| 390 | 실제 모바일 | `/solve` 차단 ✅ |

**640px에서 관리 화면 6개 전부 가로 스크롤 없음**을 확인했다 — 임계값을 낮추면서 새로 검증 범위에 들어온 구간이다.

| 화면 | 640px |
|---|---|
| `/admin/problems` · `/new` · `/excel-upload` | ✅ |
| `/admin/departments` · `/users` · `/users/excel-upload` | ✅ |

> 남은 한계: 1024px 이하 디스플레이의 200% 확대(512px)는 여전히 차단된다. WCAG 1.4.10(Reflow, 320px)도 충족하지 않는다 — 데스크톱 전용 콘솔이라 범위 밖으로 둔다.

### D6 — 포커스 표시 (§11.6) ✅ 해소

클래스 문자열을 `utils/buttonClass.js`로 뽑아 `Button`과 `<Link>`가 같은 출처를 쓰게 했다. jsdom이 없어 컴포넌트 렌더링 테스트는 불가능하지만, **순수 함수라 "모든 variant·size가 포커스 링을 포함한다"를 단위 테스트로 고정**할 수 있다.

| 화면 | 수정 전 누락 | 수정 후 |
|---|---|---|
| `/admin/problems` | 13 / 39 | **0 / 39** ✅ |
| `/admin/problems/new` | 2 / 21 | **0 / 20** ✅ |
| `/admin/problems/excel-upload` | 0 | 0 ✅ |
| `/admin/departments` · `/users` | 0 | 0 ✅ |

실제 렌더링도 확인했다.

| 요소 | outline |
|---|---|
| 표 행 "수정" 링크 | `solid` · `3px` · `rgb(0,180,227)` · offset `2px` ✅ |
| 정답 선택 라디오 | `solid` · `3px` · `rgb(0,180,227)` · offset `2px` ✅ |

`ProblemFormPage`의 "목록으로" 링크(403 분기)도 함께 고쳤다 — 브라우저 전수 조사로는 잡히지 않아 소스 검토로 찾은 지점이다.

### 테스트

| 항목 | 수정 전 | 수정 후 |
|---|---|---|
| 프론트엔드 | 171 | **179 통과** (device 2건 · buttonClass 6건 추가) |
| 프로덕션 빌드 | 성공 | 성공 |
