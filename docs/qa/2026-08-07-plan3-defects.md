...
| [D5](#d5) | **Major** | 브라우저 200% 확대 시 관리 화면에서 `/solve`로 튕겨나간다 | **수정 완료 (2026-08-09)** |
| [D6](#d6) | Minor | 문제 목록 "수정" 링크·정답 라디오에 포커스 표시가 없다 | **수정 완료 (2026-08-09)** |
...
### 결정 및 수정 (2026-08-09) — **1번 채택**

`MOBILE_BREAKPOINT`를 **768 → 640px**로 낮췄다(`frontend/src/utils/device.js`).

- 640px면 **1280px 이상 화면의 200% 확대가 모두 통과**한다. 체크리스트 기준 해상도 1440×1024의 200%(=720px)도 포함된다.
- **2번을 쓰지 않은 이유:** PRD 3.2가 *"브라우저 창 크기 조절로 경계를 넘나드는 경우에도 차단"* 을 명시한다. `pointer` 미디어 쿼리를 섞으면 판별이 뷰포트 폭 단일 신호에서 벗어나 그 요구와 충돌한다.
- **3번을 쓰지 않은 이유:** WCAG 2.1 SC 1.4.4 위반을 부채로 남기는 선택인데, 대안 1의 비용이 상수 하나를 바꾸는 것뿐이었다.
- **감수한 것:** 640~768px 구간(태블릿 세로 등)에서 관리자 화면이 열린다. PRD가 의도한 "모바일 차단" 폭이 그만큼 좁아진다. 다만 그 구간은 원래도 관리자 화면을 쓰기 어려운 폭이고, 차단의 목적이 "작은 화면에서 억지로 쓰지 않게 하는 것"임을 감안해 수용했다.

판별 근거와 대안 비교는 `device.js` 상단 주석에도 남겼다. **문서(Plan 1, 디자인 시스템 9.1)의 768px 표기는 2026-08-11에 함께 정정했다.**
