...
감사자가 실측·보고했고 **컨트롤러가 독립적으로 재현했다**(아래 대조 실험): `POST /api/admin/problems` 에
...
**컨트롤러 독립 재현(2026-08-21, 대조군 포함).** 같은 문서를 세 가지 크기로 보내 비교했다. A 와 B 의 응답이
바이트 단위로 같다는 것이 잘림의 증거다 — 1 MiB 를 더 보냈는데 서버가 아무 차이도 만들지 않았다.

| | 본문 | 결과 |
|---|--:|---|
| A 대조군 | 146 B | `400 / 1000 / "빈 보기는 입력할 수 없습니다."` |
| B 한계 초과, 앞 10 MiB 가 완결된 JSON | 11,534,485 B | `400 / 1000 / "빈 보기는 입력할 수 없습니다."` — **A 와 동일** |
| C 한계 초과, 문서가 경계에 걸침 | 11,000,046 B | `200 / 1000 / "잘못된 파라미터를 입력했습니다."` |

A = B 이므로 초과분 1,048,725 바이트는 **오류 없이 버려졌고 요청은 평범하게 실행됐다.** C 는 잘린 지점이 문서
중간이라 파싱이 깨진 경우이며, 그때조차 크기 오류가 아니라 파라미터 오류로 나온다. 즉 **어떤 경로로도 클라이언트는
"본문이 너무 크다"는 사실을 알 수 없다.**
