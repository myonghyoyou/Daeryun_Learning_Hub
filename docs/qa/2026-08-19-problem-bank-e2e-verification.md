...
| I4 | 실측 + **이월** | 5,242,881 B → 400/1015 `"이미지 크기는 5MB를 초과할 수 없습니다."`, 정확히 5,242,880 B → 200. **4~5MB 구간(플랫폼 요청 본문 상한과 겹치는 자리)은 로컬에서 잴 수 없다** — 정답지가 지시한 대로 초록으로 인증하지 않고 컷오버로 넘긴다. 다만 localhost 에도 상한이 없지는 않다는 사실을 새로 쟀다: 요청 본문이 **10 MiB(10,485,760 B)** 를 넘으면 `request.formData()` 가 먼저 실패한다(아래 "실측으로 새로 드러난 것" 1) |
...
| F7 | 실측 + **이월** | 4.2MB·5MB·8MB·9MB·10,481,664 B 전부 400/1015 `"파일 크기가 허용 범위를 초과했습니다."`. **10,485,759 B 이상은 200/1009** — 아래 1 참고 |
...
| 승인된 이탈(재심 안 함) | 7 | ①~⑦ |
| 컷오버 이월 | 3 | I4(4~5MB 플랫폼 구간) · I11ⓐ-2(Java 쪽 응답) · F7(≥10 MiB 구간, 새로 발견) |

---

## 실측으로 새로 드러난 것 (프로덕션 코드 변경 없음)

### 1. 요청 본문 10 MiB 위에서는 크기 문구(1015) 대신 1009 가 나간다 — 새 컷오버 항목

`app/api/admin/problems/excel-upload/route.ts` 와 `images/route.ts` 는 둘 다 `await request.formData()` 를 먼저 부르고,
실패하면 승인된 이탈 ⑥ 대로 `1009 "파일을 업로드할 수 없습니다."` 를 낸다. 애플리케이션 상한(엑셀 4MB / 이미지 5MB)은
그 **뒤에** 걸리므로, 본문이 런타임의 파싱 상한을 넘으면 크기 문구에 닿지 못한다. 실측 경계:

| 요청 본문 크기 | 응답 |
|---|---|
| 10,481,664 B (10 MiB − 4 KiB) | 400 / 1015 / `"파일 크기가 허용 범위를 초과했습니다."` |
| 10,485,759 B (10 MiB − 1 B) | 200 / 1009 / `"파일을 업로드할 수 없습니다."` |
| 10,485,760 B 이상 | 200 / 1009 / 같은 문구 |

즉 로컬 Node/Next 런타임의 `Request.formData()` 상한이 **정확히 10 MiB** 다(`next.config.mjs` 는 비어 있으므로 앱 설정이 아니다).
사용자 입장에서 거절되는 결과는 같지만 문구·상태·코드가 다르다. 정답지 I4 가 "플랫폼 상한값은 이 저장소 어디에도 문서화돼 있지
않으며 컷오버 시점에 실측해야 한다"고 남긴 칸의 **로컬 값**이 이것이고, 배포 플랫폼 값은 여전히 컷오버에서 재야 한다
(Vercel 은 이보다 훨씬 낮은 것으로 알려져 있어 4~5MB 구간이 문제가 되는 것이다). **코드는 손대지 않았다** — 이 순서를 뒤집으면
"파싱 실패 → 1009" 라는 형제 라우트 셋의 일관성(스탠딩 룰링)이 깨진다.

### 2. I11ⓐ-2 의 포트 쪽 절반은 이제 실측됐다
...
### 3. `problems.image_url` NOT NULL 행이 0건 → 2건
